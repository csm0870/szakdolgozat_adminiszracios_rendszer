<div class="form-modal admin-departments-add">
    <?= $this->Form->create($department, ['id' => 'departmentEditForm']) ?>
    <div class="form-header text-center">
        <?= __('Tanszék szerkesztése') ?>
    </div>
    <div class="form-body">
            <table>
                <tr>
                ...
                <tr>
                    ...
                    <td style="position: relative">
                        <?= $this->Form->control('head_of_department', ['label' => false, 'error' => false]) ?>
                    </td>
                </tr>
            </table>
    </div>
    ...
    <?= $this->Form->end() ?>
    <div class="overlay overlay-department-edit" style="display:none">
...
<script>
    $(function(){
         ...
         */
        $('#departmentEditForm .submitBtn').on('click', function(e){
            e.preventDefault();

            //Formvalidáció manuális meghívása
            if($('#departmentEditForm')[0].reportValidity() === false) return;

            $('#confirmationModal .confirmation-modal-header').text('<?= __('Biztosan mented?') ?>');
            $('#confirmationModal .modalBtn.saveBtn').text('<?= __('Mentés') ?>').css('background-color', '#71D0BD');
            //Save gomb eventjeinek resetelése cserével
            $('#confirmationModal .modalBtn.saveBtn').replaceWith($('#confirmationModal .modalBtn.saveBtn').first().clone());
            $('#confirmationModal .msg').text('<?= __('Tanszék mentése.') ?>');
            ...
            $('#confirmationModal .modalBtn.saveBtn').on('click', function(e){
                ...
                $('#departmentEditForm').trigger('submit');
            });
        });
        ...
        $('#departmentEditForm').ajaxForm({
            ...
            beforeSubmit: function(arr, $form, options) {
                $('.overlay-department-edit').show();
            },
            success: function (response, textStatus, jqXHR, $form) {
                    if(response.saved == false){
                        $('.overlay-department-edit').hide();
