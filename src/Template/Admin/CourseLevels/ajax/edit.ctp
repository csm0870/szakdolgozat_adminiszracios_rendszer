<div class="form-modal admin-courseLevels-edit">
    <?= $this->Form->create($courseLevel, ['id' => 'courseLevelEditForm']) ?>
    ...
    <?= $this->Form->end() ?>
    <div class="overlay overlay-courseLevel-edit" style="display:none">
...
<script>
    $(function(){
         ...
         */
        $('#courseLevelEditForm .submitBtn').on('click', function(e){
            e.preventDefault();

            //Formvalidáció manuális meghívása
            if($('#courseLevelEditForm')[0].reportValidity() === false) return;
            ...
            $('#confirmationModal .modalBtn.saveBtn').on('click', function(e){
                ...
                $('#courseLevelEditForm').trigger('submit');
            });
        });

        //courseLevelEditForm ajaxform
        $('#courseLevelEditForm').ajaxForm({
            replaceTarget: false,
            target: null,
            beforeSubmit: function(arr, $form, options) {
                $('.overlay-courseLevel-edit').show();
            },
            success: function (response, textStatus, jqXHR, $form) {
                    if(response.saved == false){
                        $('.overlay-courseLevel-edit').hide();
